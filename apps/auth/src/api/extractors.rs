    ...
    async fn from_request_parts(
        ...
        if let Some(user_id_header) = parts.headers.get(&X_USER_ID) {
            let user_id_str = user_id_header
                .to_str()
                .map_err(|_| ApiError::unauthorized(AuthErrorCode::InvalidCredentials.as_str(), "Некорректный заголовок X-User-Id"))?;

            let user_id = Uuid::parse_str(user_id_str)
                .map_err(|_| ApiError::bad_request(AuthErrorCode::InvalidAccessToken.as_str(), "Некорректный формат UUID"))?;
        ...
        let auth_header = parts
            ...
            .ok_or_else(|| ApiError::unauthorized(AuthErrorCode::TokenExpired.as_str(), "Токен авторизации не предоставлен"))?;

        let access_token = auth_header
            .strip_prefix("Bearer ")
            .ok_or_else(|| ApiError::unauthorized(AuthErrorCode::InvalidAccessToken.as_str(), "Неверный формат заголовка Authorization"))?;
        ...
        let session = get_session(&mut redis_conn, access_token)
            ...
            .ok_or_else(|| ApiError::unauthorized(AuthErrorCode::TokenExpired.as_str(), "Недействительный access token"))?;
